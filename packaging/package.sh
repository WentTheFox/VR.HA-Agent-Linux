...
# Produces <output-dir>/steamvr-ha-agent-<version>-<rid>.tar.gz containing the app plus install.sh.
set -euo pipefail
...
NAME="steamvr-ha-agent-$VERSION-$RID"
STAGE="$OUT_DIR/$NAME"

rm -rf -- "$STAGE"
mkdir -p "$STAGE/packaging"

dotnet publish "$REPO_DIR/src/SteamVRHAAgent/SteamVRHAAgent.csproj" \
    -c Release -r "$RID" --self-contained true \
    -p:Version="$VERSION" \
    -o "$STAGE/app"

cp "$REPO_DIR/install.sh" "$REPO_DIR/README.md" "$REPO_DIR/LICENSE" "$REPO_DIR/PRIVACY.md" "$STAGE/"
cp "$REPO_DIR/packaging/steamvr-ha-agent.service" "$STAGE/packaging/"
